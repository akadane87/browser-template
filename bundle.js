...
/***/ function(module, exports, __webpack_require__) {
	...
	var events_game = __webpack_require__(8);

	var onSignUp = function onSignUp(event) {
	  event.preventDefault();
	  var data = getFormFields(event.target);
	  api.signUp(data).then(ui.success).fail(ui.fail);
	};

	var onSignIn = function onSignIn(event) {
	  event.preventDefault();
	  var data = getFormFields(event.target);
	  api.signIn(data).then(ui.signInSuccess).fail(ui.fail);
	};
	...
	var onSignOut = function onSignOut(event) {
	  ...
	  $('#game_board').hide();
	  api.signOut().then(ui.signOutSuccess).catch(ui.fail);
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	var glowBall = __webpack_require__(11);

	function resetGlobalState() {
	  var fresh = new Array(9);
	...
	var onGetAllGames = function onGetAllGames(e) {
	  e.preventDefault();
	  api.getAllGames().then(ui.getGamesuccess).fail(ui.fail);
	};

	var onCreateGame = function onCreateGame(event) {
	  event.preventDefault();
	  resetGlobalState();
	  api.createGame().then(ui.createGameSuccess).fail(ui.fail);
	};
	...
	var onUpdateGame = function onUpdateGame() {
	  ...
	  api.updateGame(data).then(ui.Updatesuccess).fail(ui.fail);
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	var gameWins = function gameWins() {
	  var turn = gb.filter(function (e) {
	    return e !== "";
	  }).length % 2 === 0 ? 'x' : 'o';
	...
	function getTurn(cells) {
	  return cells.filter(function (e) {
	    return e !== "";
	  }).length % 2 === 0 ? "x" : "o";
	}
	...
	var onClick = function onClick(event) {
	  ...
	  var turn = getTurn(glowBall.vars.board);
	  //
	  var box = $(this).attr('id'); // plugs in id and returns string

	  var boxId = '#' + box; // ads # to previous id defintion

	  var i = +box.replace(/\D/g, ''); // removes letters from string & +returns integer
